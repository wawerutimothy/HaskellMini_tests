...
lengthChain (Block b _)  = 1 + lengthChain b
...
sumChain (Block ch tx)  = tx + sumChain ch
...
maxChain ch = myMax ch
  where 
    myMax GenesisBlock    = 0
    myMax (Block ch1 tx)  = max tx (myMax ch1)
...
longerChain ch1 ch2 
  | lengthChain ch1 > lengthChain ch2  = ch1
  | lengthChain ch1 == lengthChain ch2 = ch1
  | otherwise                          = ch2
...
getChainValues :: Chain Int -> [Int]
getChainValues (Block c v)     = v: getChainValues c
getChainValues GenesisBlock    = []

validChain :: Chain Int -> Bool
validChain ch       = 
  let result []     = True
      result [_]    = True
      result (x:xs) = x > head xs && result xs
    
  in
    result $ getChainValues ch
...
isPrefixOf ch1 ch2@(Block c _)
  | eqChain ch1 ch2                  = True
  | otherwise                        = isPrefixOf ch1 c
...
areCompatible ch1 ch2 = ch1 `isPrefixOf` ch2 || ch2 `isPrefixOf` ch1
...
commonPrefix (Block c1 v1) (Block c2 v2)
  | v1 == v2                           = Block (commonPrefix c1 c2) v1
  | otherwise                          = commonPrefix c1 c2

commonPrefix GenesisBlock GenesisBlock = GenesisBlock
commonPrefix (Block _ v1) GenesisBlock = Block (GenesisBlock) v1
commonPrefix GenesisBlock (Block _ v2) = Block (GenesisBlock) v2
...
hasBlockProp prop (Block c v)
  | prop v    = True
  | otherwise = hasBlockProp prop c
...
hasBlock v (Block c v2) 
  | v == v2             = True
  | otherwise           = hasBlock v c
...
uniqueBlocks ch@(Block c _) = not (eqChain ch c) && uniqueBlocks c
...
allBlockProp prop (Block c p) = prop p && allBlockProp prop c
...
maxChains []  = 0
maxChains (x:xs) = max (lengthChain x) (maxChains xs)
...
longestCommonPrefix ch1 []      = ch1
longestCommonPrefix ch1 (x:xs)  = longestCommonPrefix (commonPrefix ch1 x) xs   
...
balancesChain ch@(Block c _)  = Block (balancesChain c) $ sumChain ch
...
validBalancesChain ch = all (>= 0) (getChainValues (balancesChain ch))
...
shortenWhile prop (Block c v)
  | prop v    = shortenWhile prop c
  | otherwise = Block c v
...
replicateChain 0 _ = GenesisBlock
replicateChain n v
  | n <= 0         = GenesisBlock
  | otherwise      = Block (replicateChain (n-1) v) v
...
cutPrefix n _ | n <= 0 = GenesisBlock
cutPrefix _ GenesisBlock = GenesisBlock
cutPrefix n (Block c v) = Block (cutPrefix (n-1) c) v
